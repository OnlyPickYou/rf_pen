...
int kb_pairing_mode_detect(void)
{
	...
	{
		rf_set_power_level_index(RF_POWER_4dBm);
		return 1;
	}
#else
	if ((0x48 == kb_event.keycode[0]&&  0x40  == kb_event.keycode[1])||(0x40 == kb_event.keycode[0]&&  0x48  == kb_event.keycode[1]))
	{
		hm_paring_cnt++;
		if(hm_paring_cnt>=3)
		{
	...
	else if ((0x49 == kb_event.keycode[0]&& 0x5d  == kb_event.keycode[1] )||(0x5d == kb_event.keycode[0]&& 0x49  == kb_event.keycode[1] ))
	{
...
_attribute_ram_code_ u8 get_next_channel_with_idx(void)
{
	ll_chn_idx++;
	return ll_chn_table[ll_chn_idx&3];
}
...
_attribute_ram_code_ int	device_send_packet (u8 * p, u32 timeout, int retry, int pairing_link)
{
	...
	for (i=0; i<=retry; i += 1)
	{
		...
		device_channel = get_next_channel_with_idx();
	}
...
{
    if( (kb_status.mode_link&LINK_PIPE_CODE_OK) && kb_rf_pkt != (u8*)&pkt_km && kb_status.kb_mode!=STATE_PAIRING)
//	if( (kb_status.mode_link&LINK_PIPE_CODE_OK) && kb_rf_pkt != (u8*)&pkt_km )
    { //if link on,change to KB data pipe   kb_mode to STATE_NORMAL
    	kb_rf_pkt = (u8*)&pkt_km;
    	rf_set_tx_pipe (PIPE_KEYBOARD);
//    	kb_status.tx_retry = 2;
	...
	{  //manual paring
    	...
    	pkt_pairing.rsvd = kb_status.rsv1;

        if( kb_status.loop_cnt >= KB_MANUAL_PARING_MOST )
        { //pairing timeout,change to syncing mode
...
void kb_rf_proc( u32 key_scaned, u8 bat_low_det)
{
	static u32 kb_tx_retry_thresh = 0;

	if (kb_status.mode_link && kb_status.kb_mode != STATE_PAIRING)
	{
        if ( key_scaned ){
        	if(kb_event.cnt != 0)
        	{
