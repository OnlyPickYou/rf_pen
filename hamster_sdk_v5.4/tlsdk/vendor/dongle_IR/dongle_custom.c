...
void	custom_init (void)
{
	...
	for(int i=0;i<CUSTOM_DATA_MAX_COUNT;i++){
		p_custom_cfg = (custom_cfg_t *) (DEVICE_CUSTOMIZATION_INFO_ADDR+(i*sizeof(custom_cfg_t)));  //3f00
...
//	golden_dongle_enable           = !(p_custom_cfg->paring_type & CUSTOM_DONGLE_GOLDEN);
	auto_paring_enable             = !(p_custom_cfg->paring_type & CUSTOM_DONGLE_AUTO_PARING);
	auto_paring_enable_m2             = !(p_custom_cfg->paring_type & CUSTOM_DONGLE_AUTO_PARING_M2);
//	soft_paring_enable             = !(p_custom_cfg->paring_type & CUSTOM_DONGLE_SOFT_PARING);
	manual_paring_enable           = 1;
#else
	auto_paring_enable = 1;
#endif																		//paring time limit

	dongle_support_mouse_enable    = 0;    //default : mouse only
#if DONGLE_CUS_EN
//	dongle_support_keyboard_enable = 1;

	hw_ver = (p_custom_cfg->hw_ver==U8_MAX)?0xb0:p_custom_cfg->hw_ver;
	sw_ver = (p_custom_cfg->sw_ver==U8_MAX)?0xc0:p_custom_cfg->sw_ver;
	...
	rf_paring_enable       = manual_paring_enable;
//	rf_paring_enable = 0;
#else
	rf_paring_enable  = 1;
#endif
...
//		if ( (keyboard_paring_enable) && clock_time_exceed(rf_paring_tick, 10000000)){
//			keyboard_paring_enable = 0;
//		}
//	}

	if(manual_paring_enable || auto_paring_enable_m2){
