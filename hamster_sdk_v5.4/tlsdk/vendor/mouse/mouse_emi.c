...
#if MOUSE_EMI_4_FCC
void mouse_emi_process(rc_status_t *rc_status)
{
    ...
    emi_process( cmd , test_chn_idx, test_mode_sel, rc_status->pkt_addr, rc_cust_tx_power_emi );
}
#else
...
void mouse_emi_process(rc_status_t *rc_status)
{
    ...
    u32 cmd;    
    u32 btn_last;
	btn_last = rc_button_pull_and_detect( rc_status );
    if( rc_status->dbg_mode & STATE_TEST_EMI_BIT ){
        cmd = rc_button_process_emi( &test_chn_idx, &test_mode_sel, 0 );
        rc_button_process(rc_status);
        if( !(rc_status->dbg_mode & STATE_TEST_EMI_BIT) ){    //recover to emi rx mode
            cmd = 1;
            test_mode_sel = 2;
        }
    }
    else{
       cmd = rc_button_process_emi( &test_chn_idx, &test_mode_sel, 1 );
    }
    cmd |= !flg_emi_init;
    if( !flg_emi_init ){
        flg_emi_init = 1;
