...
rf_packet_audio_t pkt_audio = {
		sizeof (rf_packet_audio_t) - 4,	//0x10=20-4,dma_len

		sizeof (rf_packet_audio_t) - 5,	//0x0f=20-5,rf_len
		RF_PROTO_BYTE,						// 0x51, proto
		PKT_FLOW_DIR,						// 0x80, kb data flow
		FRAME_TYPE_AUDIO,				// 0x02, type
...
_attribute_ram_code_ void irq_tx_device_rx(void)
{
...
void kb_rf_init(void)
{
    ...
    if(kb_status.kb_mode == STATE_CONNECT){  //link OK ,deepsleep back, switch to data mode directly
...
{
    if( kb_status.isConnect && kb_rf_pkt != (u8*)&pkt_km ){ //if link on,change to KB data pipe   kb_mode to STATE_NORMAL
    	kb_rf_pkt = (u8*)&pkt_km;
    	rf_set_tx_pipe (PIPE_KEYBOARD);
...
void kb_rf_proc( u32 key_scaned )
{
	...
	kb_status.rf_sending = (km_dat_sending || !kb_status.isConnect);
	if(kb_status.rf_sending){
		if(HOST_LINK_LOST && kb_status.isConnect){
			kb_status.tx_retry = 5;
		}
